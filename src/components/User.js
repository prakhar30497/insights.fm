...
const Profile = () => {
  const { fetchUserTopItems, fetchUserProfile, fetchSavedAlbums, fetchFollowedArtists, fetchSavedTracks, fetchCurrentUserPlaylists, user, 
    fetchArtistData, fetchAudioFeatures, fetchAudioAnalysis, token } = useSpotify();

  const { userId } = useParams()
  
  const GET_USER_PROFILE = gql`
    query UserProfile {
      getUserProfile(token: "${token}", id: "${userId}") {
        userTaste {
          key
          value
          info
  ...
  const userImageUrl = user && user.images[0] ? user.images[0].url : ''
  ...
  useEffect(() => {
      ...
      fetchUserTopItems('tracks', 'long_term', 10).then(data => setTracks(data.items))
  ...
  useEffect(() => {
    ...
    setInsight(userProfileData && userProfileData.userTaste && userProfileData.userTaste[0].info)
  }, [userProfileData])
  ...
  const handleFilterClick = (type, index) => {
    if(type === 'artists'){
      ...
      fetchUserTopItems('artists', time_range[index], 10).then(data => setArtists(data.items))
    } else {
  ...
  return (!tracks || !artists) ? <Loader /> : (
      <div className='wrapper'>
        <div className='user-header'>
          <div className='user-profile'>
            <div className='user-image'>
              <img src={userImageUrl} alt='' onError={(e) => {
                e.target.onerror = null;
                e.target.src = "https://upload.wikimedia.org/wikipedia/commons/2/2c/Default_pfp.svg";
              }}/>
            </div>
            <div className='name-header'>
              <a className='user-name' href={user && user.external_urls.spotify} target="_blank" rel="noopener noreferrer">{user && user.display_name}</a>
            </div>
          </div>
        ...
        <div className='list-wrapper'>
          ...
          <ul className='list-content'>
                ...
                <li key={item.id+"profile"} className='list-content-item'>
                  <Link to={`/track/${item.id}`}>
                    <div className='item-image'>
                      <img src={item.album.images[2].url} alt='' onError={(e) => {
                        e.target.onerror = null;
                        e.target.src = "https://upload.wikimedia.org/wikipedia/commons/2/2c/Default_pfp.svg";
                      }}/>
                    </div>
            ...
            {tracks.length === 0 && <li className='list-content-item'><span className='item-name'>No Data</span></li>}
          </ul>
        </div>
        <div className='list-wrapper'>
          <div className='list-header'>
          ...
          <ul className='list-content'>
              ...
              return (
                <li key={item.id+"profile"} className='list-content-item'>
                  <Link to={`/artist/${item.id}`}>
                    <div className='item-image'>
                      <img src={item.images[0] && item.images[0].url} alt='' onError={(e) => {
                        e.target.onerror = null;
                        e.target.src = "https://upload.wikimedia.org/wikipedia/commons/2/2c/Default_pfp.svg";
                      }}/>
                    </div>
            ...
            {artists.length === 0 && <li className='list-content-item'><span className='item-name'>No Data</span></li>}
          </ul>
        </div>
        <div className='list-wrapper'>
          <div className='list-header'>
          ...
          <div className='list-content-genre' direction="row" spacing={1}>
            {genres.map(genre => <div key={genre} className='list-content-genre-item' label={genre}>{genre}</div>)}
          </div>
        </div>
        {userProfileData && userProfileData.userTaste && <div className='list-wrapper'>
          <div className='list-header'>
          ...
          <div className='profile-insight-grid-wrapper'>
            <div className='profile-insight-grid'>
              {userProfileData.userTaste.map(metadata => {
                return (<div className={metadata.key === selectedInsight ? 'profile-insight active-profile-insight-card' : 'profile-insight'} onClick={(e) => handleInsightClick(metadata)}>
                  <div className='profile-insight-detail-value'>{metadata.value}</div>
                  <div className='profile-insight-detail-label'>{metadata.key[0].toUpperCase() + metadata.key.slice(1)}</div>
                </div>)
              })}
            </div>
          </div>
          {insight && <div id='insight' className='insight-info'>{insight}</div>}
        </div>}
      </div>
  )
}
